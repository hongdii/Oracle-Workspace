...
/*
    계정 생성하는 구문. 일반 사용자 계정은 오직 관리자 계정에서 만들수 있음.
    생성방법[표현법]
...
/*
    ...
    사용자 계정 : DB에 대해서 질의, 갱신, 보고서 작성드으이 작업을 수행할 수 있는 계정, 업무에 필요한 최소한의 권한만 가지는 것을
    원칙으로 함.
    
    ROLE : 권한
    CONNECT : 사용자가 데이터베이스에 접속 가능하도록 하기 위한 CREATE SESSION 권한이 있는 ROLE
    RESOURCE : CREATE 구문을 통해 객체를 생성할 수 있는 권한과 , INSERT, DELETE 구문을 사용할 수 있는 권한을
    모아놓은 ROLE
*/
